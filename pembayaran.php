...
<div class="title"><h2>Data Anda</h2></div>
	...
	<input class="form-control" type="text" name="NM" value="<?=$rkt[nama_lengkap]?>" required="required"/>
	</div>
	
	<div class="element-number">
	<label class="title">Telphone<span class="required">*</span></label>
	<input class="form-control" type="text" value="<?=$rkt[telpon]?>" onKeyUp="this.value=this.value.replace(/[^0-9]/g,'')"  maxlength="12" name="HP" required="required" />
	</div>
...
<label class="subtitle">&nbsp;</label></span><label class="">Provinsi*</label>
	<select class="form-control" id="provinsi" name="provinsi">
			...
			<option value="<?=$rk[kabupaten]?>"><?=$rk[kabupaten]?></option>
			<?php
		...
		<input  type="hidden" class="form-control" name="KC" value="<?=$rk[id_kota]?>">
	
<label class="subtitle">&nbsp;</label></span><label class="">Pilih Kabupaten/Kota</label>
	<select class="form-control" id="kota" name="kota">
	<option value="<?=$rk[nama_kota]?>" ><?=$rk[nama_kota]?></option>
...
<option value="<?=$rk[id_kota]?>" ><?=$rk[kel]?></option>
...
<textarea  id='editor1' name='DESO' class="form-control" rows='7' cols='30'><?=$rk[ndeso]?></textarea>


<label class="subtitle">&nbsp;</label></span>
<label class="">Kode Pos*</label>
<input class="form-control" type="text" name="POS" maxlength="7" value="<?=$rk[kd_pos]?>" onKeyUp="this.value=this.value.replace(/[^0-9]/g,'')" required/>

<label class="subtitle">&nbsp;</label></span>
<label class="">Alamat Alternatif</label>
<input  class="form-control" type="text" name="ALMT" value="<?=$rk[alm_alt]?>" />
</div>
<i></i>
...
<table border=0 width=500 class='table'>
        ...
        <tr><td>Nama Kustomer</td><td> : <strong><?=$rk[nama_lengkap]?></strong></td></tr>
		<tr><td>No. Telpon/HP</td><td> : <strong><?=$rk[telpon]?></strong></td></tr>
	    <tr><td>Email</td><td> : <strong><?=$rk[email]?></strong></td></tr>
					      
	


        <tr><td>Alamat Pengiriman</td><td> <strong> <?=$rk[kabupaten]?>,<?=$rk[nama_kota]?><br />
		Desa/Kelurahan : <?=$rk[ndeso]?><br />
		 				   Rt/Rw : <?=$rk[rtb]?>/<?=$rk[rwb]?><br />
		 				   Kedo pos : <?=$rk[kd_pos]?> <br />
		 				   Alamat Alternatif : <?=$rk[alm_alt]?><br />
		</strong></td></tr>	
		
	    <tr><td colspan=2><div class='cetak'><a href='index.php?l=lihat&aksi=pembayaran1'>Edit Data</a></div></td></tr>
</table>
<input class="form-control" type="hidden" name="NM" value="<?=$rk[nama_lengkap]?>" required="required"/>
<input class="form-control" type="hidden" value="<?=$rk[telpon]?>" onKeyUp="this.value=this.value.replace(/[^0-9]/g,'')"  maxlength="12" name="HP" required="required" />
<input  type="hidden" name="provinsi" value="<?=$rk[kabupaten]?>" required="required"/>
<input  type="hidden"  name="KC" value="<?=$rk[id_kota]?>">
<input  type="hidden"  name="DESO" value="<?=$rk[ndeso]?>"  required/>
<input  type="hidden" name="POS" maxlength="7" value="<?=$rk[kd_pos]?>" onKeyUp="this.value=this.value.replace(/[^0-9]/g,'')" required/>
<input  type="hidden" name="OKRT" maxlength="3" value="<?=$rk[rtb]?>" onKeyUp="this.value=this.value.replace(/[^0-9]/g,'')"/>
<input  type="hidden" name="OKRW" maxlength="3" value="<?=$rk[rwb]?>" onKeyUp="this.value=this.value.replace(/[^0-9]/g,'')"/>
<input  class="form-control" type="hidden" name="ALMT" value="<?=$rk[alm_alt]?>" />
<input type="hidden" name="member" value="member" >
